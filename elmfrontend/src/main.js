...
var $author$project$Main$initModel = {
	...
	srcLoading: $author$project$Main$Loading($elm$core$Maybe$Nothing)
};
			...
			case 'MsgGotPass':
				...
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{
							passLoading: $author$project$Main$loadFromResult(result)
						}),
			...
			case 'MsgToggleHiddenBind':
				...
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{
							shownBindings: A2($author$project$Main$toggleSet, bind, model.shownBindings)
						}),
					$elm$core$Platform$Cmd$none);
			case 'MsgHideAllBinds':
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{shownBindings: $elm$core$Set$empty}),
			...
			case 'MsgSelectTerm':
				...
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{
							selectedTerm: $elm$core$Maybe$Just(term)
						}),
					$elm$core$Platform$Cmd$none);
			case 'MsgRenameTerm':
				var unique = msg.a;
				...
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{
							renames: A3($elm$core$Dict$insert, unique, name, model.renames)
						}),
					$elm$core$Platform$Cmd$none);
			case 'MsgToggleViewTypes':
				return _Utils_Tuple2(
					_Utils_update(
						model,
...
var $author$project$Trafo$applyRenames = function (r) {
	var tid = function (id) {
		var _v3 = A2($elm$core$Dict$get, id.unique, r);
		if (_v3.$ === 'Nothing') {
			return id;
		} else {
			var name = _v3.a;
	...
	while (true) {
		switch (term.$) {
			...
			case 'Lam':
				...
				return bndr.istyvar ? ne : A2($author$project$Core$Generated$Types$Lam, bndr, ne);
			case 'Let':
				var bind = term.a;
				var e = term.b;
				return A2(
...
var $author$project$Main$prepareModelView = function (model) {
	var filterTypes = function (loading) {
		if (loading.$ === 'Ready') {
			var pass = loading.a;
			return model.showTypeApplications ? $author$project$Main$Ready(pass) : $author$project$Main$Ready(
				_Utils_update(
					pass,
					{
						binds: A2($elm$core$List$map, $author$project$Trafo$eraseTypes, pass.binds)
					}));
		} else {
			return loading;
		}
			...
			return $author$project$Main$Ready(
				_Utils_update(
					pass,
					{
						binds: A2(
							$elm$core$List$map,
							$author$project$Trafo$applyRenames(model.renames),
							pass.binds)
					}));
		} else {
			return loading;
...
var $author$project$PprCoreLang$defaultState = F2(
	function (showUniqueName, selectedTerm) {
		return {indent: 0, result: _List_Nil, selectedTerm: selectedTerm, showUniqueName: showUniqueName, toplevel: true};
	});
var $author$project$State$State = function (a) {
	return {$: 'State', a: a};
};
...
var $author$project$State$andThen = F2(
	function (f, _v0) {
		var sf = _v0.a;
		return $author$project$State$State(
			function (s) {
				var _v1 = sf(s);
				var ns = _v1.a;
				var r = _v1.b;
...
var $author$project$State$get = $author$project$State$State(
	function (s) {
...
var $author$project$State$void = function (s) {
	return A2(
		$author$project$State$vndThen,
		$author$project$State$pure(_Utils_Tuple0),
		s);
};
	...
	return $author$project$State$withState(
		function (state) {
			var selected = function () {
				var _v1 = state.selectedTerm;
				if (_v1.$ === 'Just') {
					var oid = _v1.a;
					return _Utils_eq(oid.unique, id.unique);
				} else {
					return false;
				}
			}();
			var name = _Utils_ap(
				id.name,
				state.showUniqueName ? ('_' + id.uniquetag) : '');
			var constructor = function () {
				var _v0 = $elm$core$List$head(
					$elm$core$String$toList(id.name));
				if (_v0.$ === 'Just') {
					var x = _v0.a;
			...
			var classes = $author$project$PprCoreLang$concatMaybe(
				_List_fromArray(
					[
						...
						state.toplevel ? $elm$core$Maybe$Just(
			...
			[
				...
				_List_fromArray(
					[
						...
						_List_fromArray(
							[
								A3($author$project$Main$checkbox, model.showTypeApplications, $author$project$MsgTypes$MsgToggleViewTypes, 'Show type applications')
							])),
						A2(
						$elm$html$Html$li,
						_List_Nil,
						_List_fromArray(
							[
								A3($author$project$Main$checkbox, model.showUniqueName, $author$project$MsgTypes$MsgToggleUniqueName, 'Disambiguate variables')
							]))
					]))
			]));
};
var $author$project$MsgTypes$MsgHideAllBinds = {$: 'MsgHideAllBinds'};
...
var $author$project$Main$isShown = F2(
	function (model, bind) {
		return A2($elm$core$Set$member, bind.unique, model.shownBindings);
	});
					...
					[
						...
						_List_fromArray(
							[
								...
								_List_fromArray(
									[
										A2($author$project$Main$viewShownCheckbox, model, bind)
									])),
								A2(
								$elm$html$Html$ul,
								_List_fromArray(
								...
								_List_fromArray(
									[
										...
										_List_fromArray(
											[
												A3($author$project$Main$checkbox, true, $author$project$MsgTypes$MsgHideAllBinds, 'dont click!')
											]))
									]))
							]))
					]));
		...
		return A2(
			...
			_List_fromArray(
				[
					...
					A2($elm$core$List$map, go, pass.binds))
				]));
	});
var $author$project$MsgTypes$MsgRenameTerm = F2(
	function (a, b) {
		return {$: 'MsgRenameTerm', a: a, b: b};
	});
		...
		return A2(
			...
			_List_fromArray(
				[
					...
					_List_fromArray(
						[
							$elm$html$Html$text('name: ' + id.name)
						])),
					A2(
					$elm$html$Html$li,
					_List_Nil,
					...
					_List_fromArray(
						[
							$elm$html$Html$text('tag: ' + id.uniquetag)
						])),
					A2(
					$elm$html$Html$li,
					_List_Nil,
					_List_fromArray(
						[
							$elm$html$Html$text(
							'udi: ' + $elm$core$String$fromInt(id.unique))
						])),
					A2(
					$elm$html$Html$li,
					_List_Nil,
						...
						[
							...
							_List_fromArray(
								[
									...
									$elm$html$Html$Attributes$placeholder(id.name),
									$elm$html$Html$Events$onInput(
									$author$project$MsgTypes$MsgRenameTerm(id.unique))
								]),
							_List_Nil)
						])),
					A2(
					...
					_List_fromArray(
						[
							...
							$author$project$MsgTypes$MsgToggleHiddenBind(id.unique),
							'shown')
						]))
				]));
	};
...
var $author$project$Main$view = function (rawmodel) {
	...
	var body = function () {
		...
		switch (_v0.$) {
				...
				if (_v0.a.$ === 'Nothing') {
					...
					return $author$project$Main$view(
						_Utils_update(
							model,
							{
								passLoading: $author$project$Main$Ready(prevPass)
							}));
				}
			case 'Failure':
				var err = _v0.a;
			...
			default:
				var pass = _v0.a;
				var viewBind = A2($author$project$PprCoreLang$viewCoreBind, model.showUniqueName, model.selectedTerm);
				var binds = A2(
					...
					_List_fromArray(
						[
							...
							_List_fromArray(
								[
									$elm$html$Html$text(
									$elm$core$String$fromInt(pass.idx) + (': ' + pass.title))
								])),
